...
class ConvolutionalNN:
    def __init__(self, num_filters, filter_size, num_classes, hidden_layer_size):
        self.convolutional_layer_1 = ConvolutionalLayer(num_filters, filter_size, num_channels=1)  
        self.max_pooling_layer_1 = MaxPoolingLayer(2)  
        self.convolutional_layer_2 = ConvolutionalLayer(num_filters, filter_size, num_channels=num_filters)  
        self.max_pooling_layer_2 = MaxPoolingLayer(2)      
        self.fully_connected_layer_1 = FullyConnectedLayer(5 * 5 * num_filters, hidden_layer_size, "relu")
        self.fully_connected_layer_2 = FullyConnectedLayer(hidden_layer_size, num_classes, "softmax")
        self.num_classes = num_classes
                
    def feedforward(self, input):
        output = self.convolutional_layer_1.feedforward(input)
        output = self.max_pooling_layer_1.feedforward(output)
        output = self.convolutional_layer_2.feedforward(output)
        output = self.max_pooling_layer_2.feedforward(output)
        output = self.fully_connected_layer_1.feedforward(output)
        output = self.fully_connected_layer_2.feedforward(output)        
        return output
    ...
    def train(self, X, y, epochs, learning_rate):
        ...
        for epoch in range(epochs):
            ...
            for i in range(l):
                ...
                gradient = self.fully_connected_layer_2.backpropagate(gradient, learning_rate)
                gradient = self.fully_connected_layer_1.backpropagate(gradient, learning_rate)                
                gradient = self.max_pooling_layer_2.backpropagate(gradient)
                gradient = self.convolutional_layer_2.backpropagate(gradient, learning_rate)
                gradient = self.max_pooling_layer_1.backpropagate(gradient)
                self.convolutional_layer_1.backpropagate(gradient, learning_rate)
...
cnn = ConvolutionalNN(num_filters, filter_size, num_classes, hidden_layer_size)
cnn.train(X_train, y_train, epochs=epochs, learning_rate=learning_rate)
cnn.save_model('./models/cnn_multi_layer.pkl')

# cnn = ConvolutionalNN.load_model('./models/cnn_parameters_good.pkl')
